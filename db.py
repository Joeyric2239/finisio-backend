...
CREATE TABLE IF NOT EXISTS booking_status_log (
    ...
    note        TEXT,
    changed_at  TEXT DEFAULT (to_char(now(),'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
);
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    sent_at      TEXT DEFAULT (to_char(now(),'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
);
...
def init_db():
    if USE_POSTGRES:
        ...
        statements = [s.strip() for s in SCHEMA_PG.split(';') if s.strip()]
